...
if ( ! class_exists( 'Ai1ec_Tax_Meta_Class') ) :
...
class Ai1ec_Tax_Meta_Class {
  ...
  public function load_scripts_styles() {
    ...
    if (in_array($taxnow,$this->_meta_box['pages'])){
      ...
      // Enqueue Meta Box Scripts ( Not needed, we use requirejs )
  ...
  public function insert_images() {
    ...
    foreach( $_POST['attachments'] as $attachment_id => $attachment ) {
      ...
      $li   .= "<a title='" . __( 'Delete this image' ) . "' class='at-delete-file' href='#' rel='{$nonce}|{$term_id}|{$id}|{$attachment_id}'><img src='" . $this->SelfPath. "/images/delete-16.png' alt='" . __( 'Delete' ) . "' /></a>";
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_image( $field, $meta ) {
    ...
    if (is_array($meta) && isset($meta['src']) && $meta['src'] != ''){
      ...
      $html .= "<input class='button at-delete_image_button' type='button' rel='".$field['id']."' value='Delete Image' />";
    }else{
      ...
      $html .= "<input class='button at-upload_image_button' type='button' rel='".$field['id']."' value='Add Image' />";
    }
    echo $html;
    $this->show_field_end( $field, $meta );
  }
  ...
  public function save( $term_id ) {
  	// If this is called but the nonce field is not set, check_admin_referer() has a strange behaviour, so we just return.
  	if( basename( $_SERVER['SCRIPT_NAME'] ) === 'admin-ajax.php' && ! isset( $_POST['tax_meta_class_nonce'] ) ) {
  		return $term_id;
  	}
  ...
  public function get_tax_meta($term_id,$key,$multi = false){
    ...
    $m = Ai1ec_Meta::get_option( 'tax_meta_'.$t_id );  
    if (isset($m[$key])){
      return $m[$key];
    }else{
      return '';
  ...
  public function delete_tax_meta($term_id,$key){
    $m = Ai1ec_Meta::get_option( 'tax_meta_'.$term_id );
    if (isset($m[$key])){
      unset($m[$key]);
    }
