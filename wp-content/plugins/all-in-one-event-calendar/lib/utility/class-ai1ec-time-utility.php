...
class Ai1ec_Time_Utility
{
	...
	static public function get_local_offset_from_gmt( $timestamp ) {
		$halfday = ( (int)( $timestamp / 43200 ) ) * 43200;
		static $offsets = NULL;
		if ( NULL === $offsets ) {
			$offsets = Ai1ec_Memory_Utility::instance( __METHOD__ );
		}
		if ( NULL === ( $offset = $offsets->get( $halfday ) ) ) {
			$_this  = self::instance();
			$zone   = $_this->get_local_timezone();
			$offset = false;
			try {
				$offset = $_this->get_timezone_offset( 'UTC', $zone, $halfday );
			} catch ( Exception $tz_excpt ) {
				try {
					$offset = $this->get_gmt_offset( $halfday, $zone ) * 3600;
				} catch ( Exception $gmt_excpt ) {
					$offset = new Ai1ec_Datetime_Exception(
						'Invalid local timezone ' . var_export( $zone, true )
					);
				}
			}
			$offsets->set( $halfday, $offset );
		}
		if ( $offset instanceof Exception ) {
			throw $offset;
		}
		return $offset;
	}
