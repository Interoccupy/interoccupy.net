...
class Ai1ec_Event {
	...
	public function get_timespan_html( $start_date_display = 'long' ) {
		...
		// Get weekday of start & end dates.
		$day_start  = date_i18n( 'l', $start, true );
		$day_end    = date_i18n( 'l', $end, true );
		...
		// Find out if we need to output the end time/date. Don't output it for
		// instantaneous events and all-day events lasting only one day.
		if ( ! ( $instant || ( $this->allday && $date_start === $date_end ) ) ) {
			$output .= ' – ';

			// Decide how to display end date (if at all; hidden by default).
			...
			// Display end date, depending on $end_date_display.
			switch ( $end_date_display ) {
				case 'hidden':
					break;
				case 'weekday':
					$output .= $day_end;
